...
////// Emissive:
                ...
                if (_EnableClip == 1)
                {
                    ...
                    {
                        i.clipPos.xy = i.clipPos.xy / i.clipPos.w;
                        if (i.clipPos.x <= _ClipRange.x || i.clipPos.x >= _ClipRange.z || i.clipPos.y <= _ClipRange.y || i.clipPos.y >= _ClipRange.w)
                        {
                            return fixed4(0, 0, 0, 0);
                        }
                    }
